...
val xvecproposal = new Gaussian(0.0,0.01)
val thetaproposal = new Gaussian(0.0,0.05)
val sigeps2proposal = new Gaussian(0.0,0.01)
...
var t = DenseVector[Double](tvecarr(0).map(_.toDouble))
val yvecarr = Source.fromFile("xvec.csv").getLines.map(_.split(",")).toArray
var y = DenseVector[Double](yvecarr(0).map(_.toDouble))

val timeseries = vec2tuples(t,y)

// run mcmc
val mcmcout = mcmc(timeseries,10000)
