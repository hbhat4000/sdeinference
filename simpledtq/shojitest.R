...
ozakilik <- function(theta,h,k,bigm,littlet,data)
{
  ...
  for (i in c(1:nrow(data)))
  {
    ...
    {
      temp = dcShoji(x=data[i,j+1],t=littlet*j,x0=data[i,j],t0=littlet*(j-1),theta,d=drift,dx=driftx,dxx=driftxx,dt=driftt,s=diffusion,log=TRUE)
      lik = lik + temp
    }
  }
  return(lik)
...
save.image(file="shojiresults.RData")
