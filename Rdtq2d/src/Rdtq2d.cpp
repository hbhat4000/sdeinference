...
// f function for 2d
static inline vec fv(const vec& chaser, const vec& gammavec, const mat& runner, double t)
{
  // figure out time series time that is closest to t
  vec temptimes = t - runner.col(0);
  unsigned int i;
  for (i=0; i<runner.n_rows; i++) if (temptimes(i) < 0) break;
  unsigned int ci = i-1;

  // interpolated runner's position
  vec ri = (runner((ci+1),span(1,2))*(t - runner(ci,0)) + runner(ci,span(1,2))*(runner((ci+1),0) - t)).t();
...
// g function for 2d
static inline vec gv(const vec& chaser, const vec& thetavec)
{
  return(thetavec);
}

mat dtq(const vec &thetavec, const vec &gammavec, const mat &runner, const vec &C1, const vec &C2, double h, int numsteps, double k, double yM)
{
  double h12 = sqrt(h);
  int M = ceil(yM/k);
  int veclen = 2*M+1;
...
#pragma omp parallel for
  for (int i=0; i<datapoints; i++)
  {
    ...
    yvec(0) = C1(i);
    yvec(1) = C2(i);
    vec ftemp = fv(yvec, gammavec, runner, runner(i,0));
    vec gtemp = gv(yvec, thetavec);
        ...
        for (int jp=(j-gamma); jp<=(j+gamma); jp++)
        {
          ...
          if ((test >= 0) && (test < nr))
          {
            ...
            if ((ipeff >= 0) && (ipeff <= 2*gamma) && (jpeff >= 0) && (jpeff <= 2*gamma))
            {
              ...
              yvec(0) = (ip-M)*k;
              yvec(1) = (jp-M)*k;
              vec sigma = h12*gv(yvec, thetavec);
...
mat PDFcheck(const vec &thetavec, const vec &gammavec, const mat &runner, double h, double k, double yM)
{
  double h12 = sqrt(h);
  int M = ceil(yM/k);
  int veclen = 2*M+1;
...
SEXP dtq2dCPP(SEXP s_thetavec, SEXP s_gammavec, SEXP s_runner, SEXP s_c1, SEXP s_c2, SEXP s_h, SEXP s_numsteps, SEXP s_k, SEXP s_yM)
{
    ...
    mat mymat = dtq(thetavec, gammavec, runner, C1, C2, h, numsteps, k, yM);
    return Rcpp::wrap( mymat );
}


SEXP GCPP(SEXP s_thetavec, SEXP s_gammavec, SEXP s_runner, SEXP s_h, SEXP s_k, SEXP s_yM)
{
    vec thetavec = Rcpp::as<arma::vec>(s_thetavec);
    vec gammavec = Rcpp::as<arma::vec>(s_gammavec);
    mat runner = Rcpp::as<arma::mat>(s_runner);
    ...
    mat Gnorm = PDFcheck(thetavec, gammavec, runner, h, k, yM);
