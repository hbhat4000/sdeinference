...
double nlf(const std::vector<double> &x, std::vector<double> &grad, void *f_data)
{
  dtq *mydtq = (dtq *) f_data;

  int rv = mydtq->resetTheta(x);
  assert(rv==0);
...
int main(void)
{
  ...
  arma::mat tsT = { { -0.5415840827467999,0.771794956425898,2.229283707038429,3.187815446790356,3.471699406411336,3.209328100735848,3.468404682111315,3.414861812315891,3.721162194337222,3.668849637029822,4.038253910411896,4.263983683161315,3.463093054001823,3.411080012141716,3.788906334492286,2.784434209397839,2.900418162633692,3.052675988720498,3.365276347181400,2.987888620913433,3.390440497780164,3.418627906673532,4.060442206486768,4.588662090136544,4.893163524512659,4.169523153481756 },
  ...
  void *mydata = (void *) mydtq;

  nlopt::opt myopt(nlopt::LD_LBFGS, 3);
  myopt.set_max_objective(nlf, mydata);
  std::vector<double> theta { 0.9, 0.83, 1.2 };
  double objval;
  myopt.set_xtol_rel(1.0e-16);
  nlopt::result myresult = myopt.optimize(theta, objval);
