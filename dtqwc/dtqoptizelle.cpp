...
int dtq::getTheta(arma::vec &theta)
{
  if (theta.n_elem != curtheta.n_elem) return 1;
  else
  {
    for (int i=0; i<curtheta.n_elem; i++) theta[i] = curtheta[i];
    return 0;
  }
}
...
int dtq::compLL(void)
{
  ...
  else
  {
    ...
    double myh12 = sqrt(myh);
    // go through each particular initial condition at each time
    // and make a Gaussian
#pragma omp parallel for
    for (int i=0; i<(ltvec-1); i++)
    {
      arma::vec thiscol = arma::zeros(ylen);
      for (int j=0; j<numts; j++)
      {
        double xi = (*odata)(i,j);
        double mu = xi + ((*f)(xi,curtheta))*myh;
        double sig = ((*g)(xi,curtheta))*myh12;
        thiscol += gausspdf(yvec,mu,sig);
      }
      dtqmat.col(i) = thiscol;
    }
    dtqmat = dtqmat/numts;
...
struct Myproblem : public Optizelle::ScalarValuedFunction <double,Optizelle::Rm>
{
  ...
  public:
    ...
    {
      arma::vec newtheta(x);
      arma::vec oldtheta(newtheta.n_elem);
      int rv = mydtq->getTheta(oldtheta);
      assert(rv==0);
      if (arma::norm(newtheta-oldtheta) >= 1.0e-15)
      {
        rv = mydtq->resetTheta(x);
        assert(rv==0);
...
int main(int argc, char** argv)
{
  if(argc != 2)
  {
    std::cerr << "dtqoptizelle <parameters>" << std::endl;
    exit(EXIT_FAILURE);
  }
  char *fname = argv[1];
  ...
  std::vector<double> x = { 0.9, 0.83, 1.2 };
  for (int i=0; i<2; i++) x[2*i] = log(x[2*i]); 

  Optizelle::Unconstrained <double,Optizelle::Rm>::State::t state(x);

  Optizelle::Unconstrained <double,Optizelle::Rm>::Functions::t fns;
  fns.f.reset(new Myproblem());

  Optizelle::json::Unconstrained <double,Optizelle::Rm>::read(fname,state);

  Optizelle::Unconstrained <double,Optizelle::Rm>::Algorithms::getMin(Optizelle::Messaging::stdout,fns,state);
  ...
  for (int i=0; i<3; i++)
  {
    if (i != 1) std::cout << exp(state.x[i]) << '\n';
    else std::cout << state.x[i] << '\n';
  }
  return 0;
}
