...
= \prod_{j=0}^L p( y_{t_j} \, | \, x_{t_j}, \sigma_\epsilon^2 ).
\end{equation}
...
\label{eqn:post2}
p(\mathbf{x}, \theta, \sigma_\epsilon^2 \, | \, \mathbf{y}) 
\propto \left[ \prod_{j=0}^L p( y_{t_j} \, | \, x_{t_j},
...
\begin{equation}
\label{eqn:markov}
p(\mathbf{x} | \theta) = p(x_0) \prod_{j=0}^{L-1} p(x_{t_{j+1}} \, | \,
x_{t_j}, \theta).
\end{equation}
...
(\ref{eqn:markov}) in a parallel, distributed fashion.
...
\end{center}
\caption{Making use of Spark using math.}
