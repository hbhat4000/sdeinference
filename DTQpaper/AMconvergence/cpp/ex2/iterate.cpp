...
{
    gsl_complex z = gsl_complex_rect(y,0.0);
    double mysech = GSL_REAL(gsl_complex_sech(z));
    return(-0.5*tanh(y)*pow(mysech,2));
}

double difffun(double y)
{
    gsl_complex z = gsl_complex_rect(y,0.0);
    return(GSL_REAL(gsl_complex_sech(z)));
}
...
#pragma omp barrier
  ...
  double err = 0.0;
  for (int i=-bigm; i<=bigm; i++) {
    double exactsol = cosh(i*k)*exp(-(pow(sinh(i*k),2)/(2.0*T)))/sqrt(2.0*datum::pi*T);
    err += abs(exactsol - phatnp1(i+bigm));
  }
  cout << k*err << '\n';
}  
