...
{
  return 1.0*(theta[0] - x);
}

double myg(const double& x, const arma::vec& theta)
{
  return 0.5*theta[1]*theta[1];
}
...
int main(void)
{
  ...
  // load data from disk!
  arma::mat traj;
  traj.load("../xrou.csv", arma::csv_ascii);
  arma::mat ts = arma::join_rows(traj.t(), traj.t());
  ...
  unsigned int n0 = 50;
  unsigned int n1 = 50;
  arma::vec theta0grid = arma::linspace<arma::vec>(1.0, 7.0, n0);
  arma::vec theta1grid = arma::linspace<arma::vec>(0.125, 0.5, n1);
  double ll = 0.0;
  ...
  for (int i0=0; i0<n0; i0++)
  {
    for (int i1=0; i1<n1; i1++)
    {
      ...
      theta[1] = sqrt(2.0*theta1grid[i1]);
      ...
      ll = mydtq->getLL();
      std::cout << theta[0] << ", " << 0.5*theta[1]*theta[1] << ", " <<  ll << '\n';
    }
  }
