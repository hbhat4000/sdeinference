...
dtqres = read.csv('./dtqou10.out',header=FALSE)
...
v = v + xlab(TeX("$\\theta_2$"))
v = v + ylab(TeX("$\\theta_3$"))
ggsave(filename="OUdtq10.pdf",plot=v,units="in",width=4,height=3)
ggsave(filename="OUdtq10.eps",plot=v,units="in",width=4,height=3)
print(v)
