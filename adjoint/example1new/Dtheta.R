Dtheta <- function(xvec, yvec, h, driftfun, difffun1, c0, hermitefuncmat)
{
    diffcoeff = c0[length(c0)]
    G = integrandmat(xvec, yvec, h, driftfun, difffun1, c0,  hermitefuncmat)
    nn = length(xvec)
    mm = length(yvec)
    ...
    c0new = c0[1:(length(c0)-1)]
    fterm = driftfun(c0new ,yvec, hermitefuncmat)
