...
gradobjfun <- function(c0, h, k, bigm, littlet, data, diffcoeff)
{
    ...
    A = integrandmat(xvec, xvec, h, driftfun, difffun1, c0, hermitefuncmat)
    
    # D is a list of number of length(c0) matrices. Each matrix is the derivalive of the kernal matrix K w.r.t. one parameter.
    D = Dtheta(xvec, xvec, h, driftfun, difffun1, c0,  hermitefuncmat)
    ...
    for (j in c(1:(ndata-1)))
    {
		...
		pdfmat[,1] = rowMeans(integrandmat(xvec, data[2:nrow(data),j], h, driftfun, difffun1, c0, hermfuninit))
		...
		gammamat = k*integrandmat(data[2:nrow(data), (j+1)], xvec, h, driftfun, difffun1, c0, hermitefuncmat)
		...
		newmat = gammamat/as.vector(likelihood)
		# index = which(newmat == 'NaN', arr.ind=T)
		# newmat[index]=0
		adj = apply(newmat, 2, sum) # this is the last adjoint vector (u_j+(F-1)/F = y equation 23b  in notes)
		...
		initderivs = Dtheta(xvec, data[2:nrow(data),j], h, driftfun, difffun1, c0, hermfuninit)

		DW = Dtheta(data[2:nrow(data),(j+1)], xvec, h, driftfun, difffun1, c0, hermitefuncmat)
