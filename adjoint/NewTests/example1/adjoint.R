...
# here we assume the diffcoeff is known and we invert only for the constant coefficient parameters in the expansion of the drift function
...
gradobjfun <- function(c0, h, k, bigm, littlet, data, diffcoeff)
{
    ...
    A = integrandmat(xvec, xvec, h, driftfun, difffun1, c0, hermitefuncmat, diffcoeff)
    
    # D is a list of number of length(c0) matrices. Each matrix is the derivalive of the kernal matrix K w.r.t. one parameter.
    D = Dtheta(xvec, xvec, h, driftfun, difffun1, c0,  hermitefuncmat, diffcoeff)
    ...
    for (j in c(1:(ndata-1)))
    {
		...
		pdfmat[,1] = rowMeans(integrandmat(xvec, data[2:nrow(data),j], h, driftfun, difffun1, c0, hermfuninit, diffcoeff))
		...
		gammamat = k*integrandmat(data[2:nrow(data), (j+1)], xvec, h, driftfun, difffun1, c0, hermitefuncmat, diffcoeff)
		...
		initderivs = Dtheta(xvec, data[2:nrow(data),j], h, driftfun, difffun1, c0, hermfuninit, diffcoeff)

		DW = Dtheta(data[2:nrow(data),(j+1)], xvec, h, driftfun, difffun1, c0, hermitefuncmat, diffcoeff)
