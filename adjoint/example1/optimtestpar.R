...
result = nloptr(x0 = theta, eval_f = logobjgrad, lb = -mybounds, ub = mybounds, opts = list("algorithm"="NLOPT_LD_LBFGS", "print_level"=3, "check_derivatives" = FALSE, "xtol_abs"=1e-3))
...
fname = paste('Adjointparsamples_', hmcdatanum, '.RData', sep = '')
save(myout, file = fname)

# # to recover Hessian
# theta = result$x0

# objfun = logobjgrad(theta)$objective
# gradfun = logobjgrad(theta)$gradient
